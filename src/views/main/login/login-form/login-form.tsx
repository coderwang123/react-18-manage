...
const LoginForm: React.FC = () => {
  ...
  const initValue = {
    ...
    phone: import.meta.env.MODE == 'dev' ? '13211112222' : '', // 东鹏订单测试
    code: import.meta.env.MODE == 'dev' ? '123456' : ''
  }
  ...
  return (
    <>
      <StyledForm form={form} initialValues={initValue} onFinish={handleSubmit} autoComplete="off" size={'large'} validateTrigger={'onBlur'}>
        <Form.Item label="" name="phone" rules={[{ required: true, message: '请输入手机号码' }]} className={'phone-wrap'}>
        ...
        <Form.Item className={'submit-wrap'}>
          <Button type="primary" block loading={loginStore.disabledLogin} disabled={loginStore.disabledLogin} htmlType="submit">
            登录
          </Button>
        </Form.Item>
      </StyledForm>
          ...
          slotCustom={() => (
            <Form.Item label="" className={'img-code-wrap'}>
              <div className={'img-code-content'}>
                ...
                <Image src={imgCodeInfo.url} onClick={() => handleGetCode(false)} preview={false} style={{ cursor: 'pointer' }}></Image>
              </div>
            </Form.Item>
          )}
        />
