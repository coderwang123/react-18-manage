...
import { fakeMenuWeb, fakeUserInfo } from '@/fake-data'
...
const useLoginStore = create<IStatesLogin & IActionsLogin>()((setState, getState) => ({
  ...
  apiUserInfo: async () => {
    ...
    getState().setCache({ userInfo: fakeUserInfo })
  },
  ...
  apiPermission: async () => {
    ...
    const allMenu =
      ...
      import.meta.env.MODE != 'dev'
        ? [...fakeMenuWeb?.filter((menu: any) => menu.menuSource != '2')]
        : [...fakeMenuWeb?.filter((menu: any) => menu.menuSource != '2')]
  ...
  setPermission() {
    ...
    consola.info(mapMenusToMenus(orgMenu))
    setState({
