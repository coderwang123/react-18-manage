export class Encrypt {
   ...
   */
  static byIdCardStrong(idCard: string) {
   ...
   */
  static byIdCardCommon(idCard: string) {
    const reg = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/; // 身份证号码为15位或者18位，15位时全为数字，18位前17位为数字，最后一位是校验位，可能为数字或字符X
   ...
   */
  static byMobile(mobile: string, startIndex = 3, endIndex = 4) {
    if (!mobile) return "-";
   ...
   */
  static byTelPhone(telPhone: string, startIndex = 4, endIndex = 4) {
    if (!telPhone) return "-";
   ...
   */
  static byEmail(email: string, jmNum = 3) {
    if (!email) return "-";
   ...
   * @param endIndex
   */
  static byBank(bankCardNum: string, startIndex = 6, endIndex = 4) {
   ...
   */
  static byCommon(content: string) {
    if (!content) return "-";
